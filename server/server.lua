...
RegisterNetEvent('fivem-appearance:saveOutfit', function(name, pedModel, pedComponents, pedProps)
	local Player = QBCore.Functions.GetPlayer(source)
	MySQL.insert('INSERT INTO `players_outfits` (citizenid, name, ped, components, props) VALUES (?, ?, ?, ?, ?)', {Player.PlayerData.citizenid, name, json.encode(pedModel), json.encode(pedComponents), json.encode(pedProps)})
end)

RegisterNetEvent('fivem-appearance:getOutfit', function(name)
	local src = source
	local Player = QBCore.Functions.GetPlayer(src)
	local outfit = MySQL.Sync.fetchAll('SELECT outfit FROM players_outfits WHERE citizenid = @citizenid AND name = @name', {['@citizenid'] = Player.PlayerData.citizenid, ['@name'] = name})
...
RegisterNetEvent('fivem-appearance:getOutfits', function()
	...
	local Player = QBCore.Functions.GetPlayer(src)
	local myOutfits = {}
	local result = MySQL.Sync.fetchAll('SELECT id, name, ped, components, props FROM players_outfits WHERE citizenid = ?', {Player.PlayerData.citizenid})
	for i=1, #result, 1 do
...
RegisterNetEvent('fivem-appearance:deleteOutfit', function(id)
	local Player = QBCore.Functions.GetPlayer(source)
	MySQL.query('DELETE FROM players_outfits WHERE id = ?', {id})
end)
...
RegisterNetEvent('fivem-appearances:buyclothing', function()
	...
	if Config.Money > 0 then
		if Player.Functions.RemoveMoney('cash', Config.Money, "clothing-buy") then
			...
			TriggerClientEvent('QBCore:Notify', src, 'Nemáte dost peněz!', 'error')
